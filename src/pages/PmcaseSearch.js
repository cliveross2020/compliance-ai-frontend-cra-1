  ...
  return (
    <div className="pmcase-container">
      ...
      <form onSubmit={handleSearch} className="pmcase-form">
        <input type="text" value={query} placeholder="Search title, summary, etc..."
          onChange={(e) => setQuery(e.target.value)} className="pmcase-input" />
        ...
        <input type="text" value={filters.codeYear} placeholder="Code Year (e.g. 2021)"
          onChange={(e) => setFilters({ ...filters, codeYear: e.target.value })}
          className="pmcase-input" />

        <input type="text" value={filters.clause} placeholder="Clause (e.g. 5.1)"
          onChange={(e) => setFilters({ ...filters, clause: e.target.value })}
          className="pmcase-input" />

        <select value={filters.appeal}
        ...
        {cases.map((c, idx) => {
          ...
          return (
            <div key={idx} className="pmcase-card">
              <h3>{c.case_number || 'Untitled'}</h3>
              <p><strong>Title 1:</strong> {c.title || 'N/A'}</p>
              ...
              <p>
                ...
                {!c.summary_short ? 'N/A' :
                  isExpanded ? c.summary : c.summary_short
                }
              </p>
              {c.summary && c.summary.length > c.summary_short?.length && (
