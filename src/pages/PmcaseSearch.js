...
import './styles.css'; // Make sure your global styles or page styles are applied
...
function PmcaseSearch() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(`/api/cases/search?query=${encodeURIComponent(query)}`);
            ...
            <tbody>
              {results.map((caseItem, idx) => (
                <tr key={idx} className="border-t text-sm">
                  ...
                  <td className="p-2">
                    <a href={caseItem.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                      View
                    </a>
                  </td>
                </tr>
