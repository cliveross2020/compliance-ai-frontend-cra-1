...
export default function AskCaiWorkbench() {
    ...
    const [preset, setPreset] = useState("abpi");
    ...
    async function ask() {
        ...
        try {
            ...
            // Make clause refs clickable
            processedAnswer = processedAnswer.replace(
                /Clause\s+(\d+(\.\d+)?)/gi,
                `<a href="#" class="clause-link" data-clause="$1">Clause $1</a>`
            );
    ...
    // Handle clickable clause links
    function handleClauseClick(e) {
        if (e.target.classList.contains("clause-link")) {
            e.preventDefault();
            const clause = e.target.getAttribute("data-clause");
            if (clause) {
                searchPluginInstance.jumpToMatch({
    ...
    return (
        <div className="workbench" onClick={handleClauseClick}>
            {/* PDF */}
