...
/** Prefer UK ABPI 2024 if available */
...
export default function AskCaiWorkbench() {
  ...
  // PDF viewer plugin (toolbar + search, thumbnails, etc.)
  ...
  const [answer, setAnswer] = useState("");            // string (≤350 words)
  ...
  async function ask() {
    ...
    try {
      const res = await fetch(`${API}/askcai/answer`, {
        ...
        body: JSON.stringify({
          question: q,
          doc: preset,      // future-proof: swap sources later when EFPIA paragraphs land
          top_k: 6,
          max_words: 350,
        }),
      });
  ...
  return (
    <div className="workbench">
      {/* Left: PDF viewer (react-pdf-viewer) */}
      <section className="wb-left">
        ...
        <div className="wb-pdf-wrap">
          {/* Use PDF.js worker from CDN for simplicity */}
          <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.4.120/build/pdf.worker.min.js">
            <div className="wb-pdf-viewer">
        ...
        <div className="wb-hint">
          Tip: <strong>Search</strong> in the toolbar (or press <strong>⌘/Ctrl + F</strong>) to find inside the PDF.
        </div>
      </section>
      ...
      <section className="wb-right">
        ...
        {/* Clean, minimal answer — no extra headings */}
        {answer && <div className="wb-answer-body">{answer}</div>}

        {/* Clause pills (only if present) */}
        {!!usedClauses.length && (
        ...
        {/* Matches (compact list; only if present) */}
        {!!matches.length && (
