...
const HOST = RAW.endsWith("/api") ? RAW.slice(0, -4) : RAW; // backend origin without /api
...
/** Turn “(Clause 10.5)” into clickable links that will drive the viewer search */
...
export default function AskCaiWorkbench() {
  ...
  const viewerSrc = useMemo(() => {
    ...
    const base = `${HOST.replace(/\/+$/, "")}/pdfjs/web/viewer.html`; // NOTE: served from backend root (no /api)
    const qs = `file=${encodeURIComponent(proxiedPdf)}`;             // NOTE: encoded once for file=
  ...
  return (
    <div className="workbench">
      {/* Left: PDF.js viewer in an iframe (served by backend root) */}
