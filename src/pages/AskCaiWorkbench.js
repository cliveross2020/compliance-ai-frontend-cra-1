...
/** Turn “(Clause 10.5)” into clickable links that drive the viewer search */
...
export default function AskCaiWorkbench() {
  ...
  // Build a same-origin PDF url via the backend proxy
  const proxiedPdf = useMemo(() => {
    if (!pdfUrl) return "";
    return `${API}/proxy/pdf?url=${encodeURIComponent(pdfUrl)}`;
  }, [pdfUrl]);

  // The search term we want the viewer to highlight (when a clause link is clicked)
  const [searchTerm, setSearchTerm] = useState("");

  // Final viewer SRC: backend’s PDF.js + same-origin file + hash search
  const viewerSrc = useMemo(() => {
    if (!proxiedPdf) return "";
    // Prefer the /api/pdfjs alias; /pdfjs also works
    const base = `${API}/pdfjs/web/viewer.html`;
    const qs = `file=${encodeURIComponent(proxiedPdf)}`;
    const hash = searchTerm ? `#search=${encodeURIComponent(searchTerm)}` : "";
    return `${base}?${qs}${hash}`;
  }, [API, proxiedPdf, searchTerm]);
  ...
  return (
    <div className="workbench">
      {/* Left: PDF.js viewer in an iframe (served by backend) */}
      <section className="wb-left">
        ...
        <div className="wb-hint">
          The viewer has a built-in <strong>Find</strong>. When you click a clause in the
          answer, we auto-fill the search box in the viewer.
        </div>
      </section>
