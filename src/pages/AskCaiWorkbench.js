...
export default function AskCaiWorkbench() {
  ...
  const pdfUrl = useMemo(() => {
    ...
    if (preset === "efpia") {
      return (
        ALL_PDFS.find(
          (r) =>
            /Europe\s*\(EFPIA\)/i.test(r.label) && /Code of Practice/i.test(r.label)
  ...
  // --- chip prefill: read ?q= from URL and pre-populate the textarea ---
  ...
  return (
    <div className="workbench">
      {/* Left: PDF (iframe, stable & supports native Find) */}
      <section className="wb-left">
        ...
        <div className="wb-hint">
          Tip: press <strong>⌘F</strong> / <strong>Ctrl+F</strong> to find inside
          the PDF.
        </div>
      </section>
