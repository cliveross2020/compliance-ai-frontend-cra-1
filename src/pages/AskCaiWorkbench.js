...
function ensureAdobeSdk() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (document.getElementById(id)) {
      const tick = () => (window.AdobeDC ? resolve() : setTimeout(tick, 80));
      tick();
      return;
    }
...
export default function AskCaiWorkbench() {
  ...
  // Viewer state
  const [useAdobe, setUseAdobe] = useState(true); // prefer Adobe
  const [loadError, setLoadError] = useState("");
  const containerRef = useRef(null); // outer scrollable container
  const viewerRef = useRef(null);    // Adobe inner div
  ...
  async function renderWithAdobe(bytes) {
    ...
    // Make absolutely sure the container scrolls (CSS may be cached)
    containerRef.current.style.overflow = "auto";
    containerRef.current.style.height = "calc(100vh - 220px)";
      ...
      try {
        ...
        if (useAdobe) {
          try {
            ...
            setUseAdobe(false); // fallback to native PDF toolbar
            setLoadError(`Adobe viewer unavailable: ${e.message || e}. Using native viewer.`);
          }
        }
      } catch (e) {
  ...
  // Clause click → copy clause text for quick find
  useEffect(() => {
  ...
  async function ask() {
    ...
    try {
      const res = await fetch(`${API}/askcai/answer`, {
        ...
        body: JSON.stringify({ question: q, pdf_url: pdfUrl, top_k: 6, max_words: 350 }),
      });
  ...
  return (
    <div className="workbench">
      ...
      <section className="wb-left">
          ...
          <div className="wb-doc-pickers">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setUseAdobe(true); // try Adobe again on change
                setLoadError("");
              }}
              className="wb-select"
            >
        ...
        <div
          className="wb-pdf-wrap"
          ref={containerRef}
          style={{ height: "calc(100vh - 220px)", overflow: "auto" }}
        >
          {/* Adobe occupies this container when useAdobe === true.
              If Adobe fails, we render a native iframe with toolbar & nav. */}
          {!useAdobe && proxiedPdf && (
            <iframe
              title="Code PDF"
              className="wb-pdf"
          ...
          {!proxiedPdf && <div className="wb-pdf-placeholder">Select a document…</div>}
        </div>
