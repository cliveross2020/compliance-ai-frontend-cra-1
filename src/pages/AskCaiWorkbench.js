...
const RAW = process.env.REACT_APP_BACKEND_URL || "https://compliance-ai-app.onrender.com";
...
export default function AskCaiWorkbench() {
  ...
  async function ask() {
    ...
    try {
      const res = await fetch(`${API}/askcai/answer`, {
        ...
        body: JSON.stringify({
          question: q,
          pdf_url: pdfUrl, // <- we still send the chosen PDF to backend for retrieval
          top_k: 6,
          max_words: 350,
        }),
      });
  ...
  /** Initialize Adobe viewer, and re-render when pdfUrl changes */
  useEffect(() => {
    let cancelled = false;

    async function init() {
      if (!ADOBE_CLIENT_ID) {
        console.warn("Missing REACT_APP_ADOBE_EMBED_CLIENT_ID; showing simple fallback message.");
        return;
      }
      try {
        ...
        // Wait for SDK ready event if needed
        ...
        adobeViewRef.current
          .previewFile(
            {
              content: { location: { url: pdfUrl } }, // public URL works (no proxy required)
              metaData: { fileName: (pdfUrl.split("/").pop() || "document.pdf").split("?")[0] },
            },
            {
          ...
          .then((viewer) => {
            // Try to grab APIs (not all plans expose every API)
        ...
        console.error("Failed to load Adobe SDK:", err);
      }
    }
  ...
  useEffect(() => {
    ...
    const onClick = async (e) => {
      ...
      // Try programmatic search if available; otherwise copy to clipboard & instruct user
      try {
        if (adobeApisRef.current && adobeApisRef.current.search) {
          // Some plans expose a search API. If yours does, this will work:
    ...
    <div className="workbench">
      ...
      <section className="wb-left">
        ...
        <div className="wb-hint">
          The viewer has a built-in <strong>Find</strong>. Click a cited clause to jump-search (or we’ll copy it for ⌘F).
        </div>
      </section>
