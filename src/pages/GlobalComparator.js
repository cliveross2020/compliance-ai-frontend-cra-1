...
const GlobalComparator = () => {
  ...
  const [expanded, setExpanded] = useState({}); // track open snippets
    ...
    <div className="gc-layout">
      ...
      <div className="gc-form">
        <div className="gc-logo-wrap">
          <img src={logo} alt="CAI Logo" className="gc-logo" />
        </div>
        ...
        <button
          ...
          disabled={
            loading ||
            !topic.trim() ||
            ![country1, country2, country3].some(Boolean)
          }
        >
      ...
      <div className="gc-content">
        ...
        {!error && !loading && Object.keys(results).length === 0 && (
          <div className="gc-muted">
            Pick up to three countries and a topic, then hit{" "}
              ...
              <div className="gc-card" key={country}>
                <div className="gc-card-head">
                  <div className="gc-card-title">
                    ...
                    {info.code && <span className="gc-code"> — {info.code}</span>}
                  </div>
                  {info.url && (
                    <a
                      className="gc-link"
                  ...
                  <ul className="gc-list">
                    {(info.matches || []).map((m, i) => {
                      ...
                      return (
                        <li
                          key={i}
                          className={`gc-snippet ${isOpen ? "open" : ""}`}
                          onClick={() => toggleExpand(key)}
                        >
                          {!isOpen ? (
                            <>
                              {highlight(m, topic.trim())}
                              <span className="gc-more"> (click to expand)</span>
                            </>
                          ) : (
                            <>
                              <div className="gc-full-context">
                                {highlight(m, topic.trim())}
                              </div>
                              <div className="gc-more"> (click to collapse)</div>
                            </>
                          )}
                        </li>
                      );
